			...
			<div class="container_12">
				<div class="widget-panel grid_12">
					<div class="widget-top">
						<h4>Agregar Programa Universidad</h4>
					</div>
					<div class="widget-content module">
						<div class="form-grid">
							...
							<form method="post" name="form1" id="form1" action="guardar_programa_universidad.php" class="topLabel">
								<ul>
									<li>
									  <label class="fldTitle">Universidad<abbr title="Required Field" class="require">*</abbr></label>
									  <div class ="fieldwrap">
										<select id="iduniversidad" name="iduniversidad" class="chzn-select full" tabindex="11">
                                            <?php 
											
												$consulta = "SELECT u.iduniversidad, e.estado, u.nombre FROM universidad u, estado e WHERE e.idestado = u.idestado AND  NOT EXISTS (SELECT * FROM programas_has_universidad pu WHERE pu.iduniversidad = u.iduniversidad) ORDER BY e.estado, u.nombre";
												...
												{
											?>
												<option value="<?php echo $row['iduniversidad'];?>"><?php echo utf8_encode($row['estado'].' - '.$row['nombre']);?></option>
                                            <?php 
									...
									<div class ="fieldwrap">
										<select data-placeholder="Elige un Programa..." id="idprogramas" name="idprogramas" tabindex="15">
                                            <?php 
											
												$consulta2 = "SELECT idprogramas, programas FROM programas WHERE estatus = 1 ORDER BY programas";
												$resultado2 = $db->consulta($consulta2);
												$row2 = $db->fetch_assoc($resultado2);
												do{
												?>
                                                	
                                            		<option value="<?php echo $row2['idprogramas']?>"><?php echo utf8_encode($row2['programas']);?></option>
                                        	<?php 
                                    ...
                                    <li class="buttons bottom-round noboder">
                                    	<input name="bAgregarp" id="bAgregarp" value="Agregar Programa" type="button" onClick="agregarProgramasUni('idprogramas','d_programasUni')" class="submit-button">
                                    </li>
                                    <li>
                                    	<div id="d_programasUni" class="widget-content module">
                                        	<?php $pro->mostrarProgramas(0); ?>
                                        </div>
                                    </li>
									<li class="buttons bottom-round noboder">
									<div class ="fieldwrap">
										<!--<input name="" value="Limpiar Formulario" type="button" onClick="eliminarSesion()" class="submit-button">-->
                                        <input name="" type="submit" value="Guardar" class="submit-button">
									</div>
									</li>
								</ul>
							</form>
